...
*/
var base58check$1 = base58check(sha256);
...
var randomSecretKey = typeof secpUtils.randomSecretKey === "function" ? (seed) => secp256k1.utils.randomSecretKey(seed) : (seed) => secpUtils.randomPrivateKey(seed);
...
var HDKey = class HDKey {
	...
	set publicKey(value) {
		...
		if (!secp256k1.utils.isValidPublicKey(publicKey)) throw new Error("Invalid public key");
		this.setPublicKey(publicKey);
	}
	get privateExtendedKey() {
		if (this._privateKey) return base58check$1.encode(this.serialize(this.versions.private, concatBytes(new Uint8Array([0]), this._privateKey)));
		return null;
	}
	get publicExtendedKey() {
		return base58check$1.encode(this.serialize(this.versions.public, this.publicKey));
	}
	derive(path) {
	...
	deriveChild(index) {
		...
		if (isHardened) {
			...
			data = concatBytes(concatBytes(new Uint8Array([0]), this.privateKey), indexBuffer);
		} else data = concatBytes(this.publicKey, indexBuffer);
		const I = hmac(sha512, this.chainCode, data);
		const IL = I.slice(0, 32);
		const IR = I.slice(32);
		const hdkey = new HDKey(this.versions);
		if (this.privateKey) try {
			const sum = (BigInt("0x" + bytesToHex(this.privateKey)) + BigInt("0x" + bytesToHex(IL))) % CURVE_ORDER;
	...
	static fromMasterSeed(seedBuffer, versions) {
		const I = hmac(sha512, MASTER_SECRET, seedBuffer instanceof Buffer ? new Uint8Array(seedBuffer) : seedBuffer);
		const IL = I.slice(0, 32);
		const IR = I.slice(32);
	...
	static fromExtendedKey(base58key, versions, _skipVerification) {
		...
		const hdkey = new HDKey(versions);
		const keyBuffer = base58check$1.decode(base58key);
...
//#endregion
export { HARDENED_OFFSET, HDKey, HDKey as default };
